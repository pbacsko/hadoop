...
public class TestAppNameMappingPlacementRule {
  ...
  private void createQueueHierarchy(CapacitySchedulerQueueManager queueManager) {
    ...
    when(queueManager.isAmbiguous(AMBIGUOUS_QUEUE)).thenReturn(true);
  }

  private void verifyQueueMapping(QueueMapping queueMapping,
      String user, String expectedQueue) throws IOException, YarnException {
  ...
  private void verifyQueueMapping(QueueMapping queueMapping,
      ...
      boolean overwrite) throws IOException, YarnException {
  ...
  CSMappingPlacementRule setupEngine(MappingRule rule,
    ...
    when(queueManager.isAmbiguous(Mockito.isA(String.class))).thenReturn(false);
